...
fn overflowing_add(a: u64, b: u64, carry: bool) -> (u64, bool) {
    ...
    if sum >= a {
        if carry {
            let carried_sum = sum.wrapping_add(1);
            (carried_sum, carried_sum <= sum)
        } else {
            (sum, false)
        }
    } else {
...
mod tests {
    ...
    fn test_add() {
        ...
        let max = !0 as u64;
        let b63 = BigInt::from_vec(vec![max, max, max]);
        let b001 = BigInt::from_vec(vec![1]);
        assert_eq!(&b001 + &b63, BigInt::from_vec(vec![0, 0, 0, 1]));
        assert_eq!(&b63 + &b001, BigInt::from_vec(vec![0, 0, 0, 1]));
        assert_eq!(BigInt::new(0) + BigInt::new(0), BigInt::new(0));
    }
}
